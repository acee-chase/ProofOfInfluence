...
export class UserVaultService {
   ...
   * Ensures the demo user exists before creating the vault to avoid foreign key constraint errors
   */
  async getOrCreateDemoVault(demoUserId: string): Promise<UserVault> {
    // Step 1: Ensure demo user exists (avoid foreign key constraint error)
    let user = await storage.getUser(demoUserId);
    if (!user) {
      user = await storage.upsertUser({
        id: demoUserId,
        role: "user",
        // Note: email, walletAddress, etc. are optional for demo users
      });
    }

    // Step 2: Try to find existing vault
    ...
    // Step 3: Create new vault (user now exists, so foreign key constraint will pass)
