...
export class TestScenarioRunner {
  ...
  async runImmortalityPlayableAgent(
    ...
    const errors: string[] = [];
    let testWallet: { id: string; walletAddress: string } | null = null;
    ...
    try {
      for (let i = 0; i < wallets; i++) {
        try {
          ...
          // Link wallet to user
          // Note: Test wallet status tracking is handled by testWalletService
          // TODO: When UserVault schema is ready, link wallet to vault
